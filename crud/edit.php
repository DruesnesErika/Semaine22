...
$produit = strip_tags($_POST['pro_libelle']);
$reference = strip_tags($_POST['pro_ref']);
$categorie = strip_tags($_POST['cat_nom']);
$description = strip_tags($_POST['pro_description']);
$prix = strip_tags($_POST['pro_prix']);
$stock = strip_tags($_POST['pro_stock']);
$couleur = strip_tags($_POST['pro_couleur']);

$sql = "UPDATE'produits' SET 'pro_libelle' =:pro_libelle, 'pro_ref'=:pro_ref, 'pro_description'=:pro_description, 'pro_prix'=:pro_prix, 'pro_stock'=:pro_stock, 'pro_couleur'=:pro_couleur WHERE 'pro_id'=:pro_id;
UPDATE 'categories' SET 'cat_nom'=:cat_nom WHERE 'pro_id'=:pro_id;";
...
$id = strip_tags($_GET['pro_id']);

$sql = "SELECT pro_id, pro_ref, cat_nom, pro_libelle, pro_description, pro_prix, pro_stock, pro_couleur, pro_d_ajout, pro_d_modif FROM produits, categories WHERE produits.pro_cat_id = categories.cat_id AND 'id' = :pro_id;"; 
...
<body>
    <div class="container">
        ...
        <div class="form-group">
            ...
            <input type="text" id="pro_libelle" name="pro_libelle" class="form-control" value="<?= $produit['pro_libelle']?>">
        </div>
        <div class="form-group">
            <label for="pro_ref">Référence du produit</label>
            <input type="text" id="pro_ref" name="pro_ref" class="form-control" value="<?= $produit['pro_ref']?>">
        </div>
        <div class="form-group">
            <label for="cat_nom">Catégorie</label>
            <input type="text" id="cat_nom" name="cat_nom" class="form-control"value="<?= $produit['cat_nom']?>">
        </div>
        <div class="form-group">
            <label for="pro_description">Description</label>
            <input type="text" id="pro_description" name="pro_description" class="form-control"value="<?= $produit['pro_description']?>">        </div>
        <div class="form-group">
            <label for="pro_prix">Prix</label>
            <input type="text" id="pro_prix" name="pro_prix" class="form-control"value="<?= $produit['pro_prix']?>">
        </div>
        <div class="form-group">
            <label for="pro_stock">Stock</label>
            <input type="number" id="pro_stock" name="pro_stock" class="form-control"value="<?= $produit['pro_stock']?>">
        </div>
        <div class="form-group">
            <label for="pro_couleur">Couleur</label>
            <input type="text" id="pro_couleur" name="pro_couleur" class="form-control"value="<?= $produit['pro_couleur']?>">
        </div>
                <input type="hidden" value="<?= $produit['pro_id']?>" name="pro_id">
